...
class BaseVectorStore(ABC):
    ...
    def as_retriever(self, top_k: int = 4, filters: Optional[Dict[str, Any]] = None) -> BaseRetriever:
        """LangChain BaseRetriever로 변환"""
        return VectorStoreRetrieverAdapter(store=self, top_k=top_k, filters=filters)
    ...
    def _get_relevant_documents(
        ...
        *,
        ...
        return asyncio.get_event_loop().run_until_complete(
            self.store.search(query, top_k=self.top_k, filters=self.filters)
        )
